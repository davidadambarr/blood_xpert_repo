...
This document is an Rmarkdown knitted as a pdf. This means alll the code for the analysis from raw data to final figures is embedded (so 100% reproducible). The code chunks are suppressed for readability but available at git repository [url: ].  
...
[ *DB: Accepted practice when doing a systematic review is to search multiple databases. But this inevitably means searching a commercial database i.e. a database maintained by a for-profit organisation which requires a subscription. UoL has access to Scopus so have used this, but I find it less transparent how the search query works in Scopus.* ]  
